// src/components/TileCard.jsx — reusable tile card
"use client";
    ...
    <div className="tile-card bg-white rounded-2xl overflow-hidden border border-clay-100 shadow-sm flex flex-col">
      ...
      <div className="p-4 flex flex-col gap-2 flex-1">
        ...
        <div className="flex gap-1.5 flex-wrap">
          <span className="bg-clay-100 text-clay-700 text-xs px-2 py-0.5 rounded-full">{tile.material}</span>
          <span className="bg-clay-100 text-clay-700 text-xs px-2 py-0.5 rounded-full">{tile.dimensions}</span>
        </div>

        <p className="text-gray-400 text-xs leading-relaxed line-clamp-2 flex-1">{tile.description}</p>

        {/* Price + button */}
        <div className="flex items-center justify-between pt-3 border-t border-clay-50 mt-auto">
          <div>
            <p className="text-xs text-gray-400">From</p>
            <p className="font-display font-semibold text-clay-700">${tile.price}</p>
          </div>
          <Link
            href={`/tile/${tile.id}`}
